...
import sqlite3
import re
from scrapers import scrape_foxsports as sfox
...
def insert_into_db(data):
    ...
    away_team_id = data['gameInfo']['visiting-team']['team-code']['id']
    away_team_city = data['gameInfo']['visiting-team']['team-city']['city']
    away_team_alias = data['gameInfo']['visiting-team']['team-name']['alias'].upper()
    away_team_name = data['gameInfo']['visiting-team']['team-name']['name']
    away_team_info = [away_team_id, away_team_city, away_team_alias, away_team_name]    
    ...
    with sqlite3.connect(db_filename) as conn:
        ...
        cursor.execute("SELECT id, game_id FROM schedule WHERE game_date=? AND home_team_id=? AND away_team_id=?", (game_date, home_team, away_team))
        prev_schedule = cursor.fetchone()
        cursor.execute("SELECT id FROM game WHERE game_code=?", (game_code,))
        prev_game = cursor.fetchone()
        
        if prev_schedule:
            if not prev_game:
                cursor.execute('INSERT OR IGNORE INTO game (season, game_date, home_team, away_team, game_type, game_code) VALUES (' + ','.join(['?'] * len(game_info)) + ')', game_info)
            game_id = prev_game[0] if prev_game else cursor.lastrowid
            schedule_info = [home_score, away_score, game_id, game_date, home_team, away_team]
            cursor.execute('UPDATE schedule SET home_team_score=?, away_team_score=?, game_id=? '
                           'WHERE game_date=? AND home_team_id=? AND away_team_id=?' , schedule_info)
        else:
        ...
        if not prev_game:
            ...
            cursor.execute('INSERT OR IGNORE INTO team (id, city, alias, name) VALUES (' + ','.join(['?'] * len(away_team_info)) + ')', away_team_info)
        ...
        if not prev_game:
            for play in data['pbp']['sports-scores']['nba-scores']['nba-playbyplay']['play']:
                ...
                insert = sfox.insert_play(play['textual-description'])
                insert.update(play)
